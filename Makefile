...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "evoseq", "tests")); system.time(test_check("evoseq", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran evoseq_$(VERSION).tar.gz
	rm -rf evoseq.Rcheck/
